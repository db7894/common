...
Then curl localhost:16384 to get a list of stack frames and call counts. To
create flame graphs, grab the netflix package and::
...
def run_profiler(host='0.0.0.0', port=16384):
    sampler = Sampler()
    sampler.start()
    emitter = Emitter(sampler, host, port)
    emitter.run()
