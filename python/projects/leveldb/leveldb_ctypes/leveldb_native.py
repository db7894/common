...
# leveldb_comparator
DestructorFunction = ctypes.CFUNCTYPE(None, ctypes.c_void_p)
ComparisonFunction = ctypes.CFUNCTYPE(ctypes.c_int, ctypes.c_void_p, ctypes.c_char_p, ctypes.c_size_t, ctypes.c_char_p, ctypes.c_size_t)
StringGetFunction  = ctypes.CFUNCTYPE(ctypes.c_char_p, ctypes.c_void_p)

LDB.leveldb_comparator_create.argtypes = [
    ctypes.c_void_p, DestructorFunction, ComparisonFunction, StringGetFunction]
LDB.leveldb_comparator_create.restype = ctypes.c_void_p

LDB.leveldb_comparator_destroy.argtypes = [ctypes.c_void_p]
LDB.leveldb_comparator_destroy.restype  = None
...
def get_destructor_callback(method):
    return DestructorFunction(method)

def get_comparison_callback(method):
    return ComparisonFunction(method)

def get_string_get_callback(method):
    return StringGetFunction(method)
