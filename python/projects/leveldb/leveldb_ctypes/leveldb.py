...
from leveldb_native import get_destructor_callback
from leveldb_native import get_comparison_callback
from leveldb_native import get_string_get_callback
...
class DatabaseComparator(object):

    def __init__(self, name=b'default'):
        self.name = get_string_get_callback(lambda _: name)
        self.method = get_comparison_callback(self.compare)
        self.destructor = get_destructor_callback(self.destruct)
        self.comparator = LDB.leveldb_comparator_create(None, self.destructor, self.method, self.name)

    @staticmethod
    def compare(state, a, a_len, b, b_len):
        if (a_len == b_len) and (a == b): return 0
        if (a_len > b_len)   or (a >= b): return 1
        if (a_len < b_len)   or (a <  b): return -1
...
if __name__ == "__main__":
    ...
    comparator = None
    with Database(filename='temporary', comparator=comparator) as database:
    ...
    Database(filename='temporary', comparator=comparator).destroy()
