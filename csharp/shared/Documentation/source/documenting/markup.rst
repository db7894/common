...
they are used in the Bashwork documentation.
...
Examples of source code or interactive sessions are represented using
standard reST literal blocks.  They are started by a ``::`` at the end of the
preceding paragraph and delimited by indentation.

Representing an interactive session requires including the prompts and output
along with the code.  No special markup is required for interactive
...
* There is a "highlighting language" for each source file.  Per default,
  this is ``'csharp'`` as the majority of files will have to highlight C-Sharp
  snippets.

* Within C-Sharp highlighting mode, interactive sessions are recognized
...
``literalinclude`` directive. [1]_ For example, to include the source file
:file:`example.cs`, use::
