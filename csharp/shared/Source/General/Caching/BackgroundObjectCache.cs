...
namespace SharedAssemblies.General.Caching
{
	...
	public class BackgroundObjectCache<TValue> : IObjectCache<TValue>
	{
		...
		private readonly IObjectCacheProvider<TValue> _provider;
		...
		/// <param name="provider">The provider for the backing store</param>
		public BackgroundObjectCache(Func<TValue> factory, TimeSpan rate,
			IObjectCacheProvider<TValue> provider = null)
		{
			...
			_provider = provider ?? new MemoryObjectCacheProvider<TValue>();
			Refresh();
		}
