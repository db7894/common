...
namespace Bashwork.General.Validation.Rules
{
	...
	{
		/// <summary>
		/// Validation rule that tests if the supplied string is not null or empty.
		/// </summary>
		...
		public static IPropertyContext<TObject, string> IsNotNullOrEmpty<TObject>(
			this IPropertyContext<TObject, string> context)
		{
			context.Guard(MessageResources.NotNullPropertyContext);
			context.Obeys(property => string.IsNullOrEmpty(property).Not());
			return context;
		}
